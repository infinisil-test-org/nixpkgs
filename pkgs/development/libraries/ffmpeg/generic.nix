...
stdenv.mkDerivation (finalAttrs: {
  ...
  patches = map (patch: fetchpatch patch) (
    ...
    ++ (lib.optional
      (lib.versionAtLeast finalAttrs.version "6" && lib.versionOlder finalAttrs.version "6.1")
      {
    ...
    ++ (lib.optional
      (lib.versionAtLeast finalAttrs.version "6.1" && lib.versionOlder finalAttrs.version "6.2")
      {
        # this can be removed post 6.1
  ...
  configureFlags =
    ...
    ++ optionals (versionAtLeast finalAttrs.version "5.0") [ (enableFeature withPlacebo "libplacebo") ]
    ++ [
    ...
    ++ optionals (versionAtLeast finalAttrs.version "5.0") [ (enableFeature withShaderc "libshaderc") ]
    ++ [
  ...
  buildInputs =
    ...
    ++ optionals (withNvdec || withNvenc) [
      (if (lib.versionAtLeast finalAttrs.version "6") then nv-codec-headers-12 else nv-codec-headers)
    ]
    ...
    ++ optionals withPlacebo [
      (if (lib.versionAtLeast finalAttrs.version "6.1") then libplacebo else libplacebo_5)
      vulkan-headers
    ]
