      ...
      passthru =
        if (dedicatedServer) then
        ...
        else if (version != srcs.${latestVersionMajor}.version) then
          {
            ...
            dedicated = mkArmagetron (params // { dedicatedServer = true; });
          }
        else
          (lib.mapAttrs (name: value: mkArmagetron value) (
            lib.filterAttrs (name: value: value.version != srcs.${latestVersionMajor}.version) srcs
          ))
          // {
