...
rec {
  ...
  overrideLibcxx =
    ...
    let
      ...
      cxxabi =
        pkgs.runCommand "${stdenvCxxabi.name}-${llvmLibcxxVersion}"
          {
            outputs = [
              "out"
              "dev"
    ...
    stdenv.override (old: {
      ...
      mkDerivationFromStdenv = withOldMkDerivation old (
        ...
        (mkDerivationSuper args).overrideAttrs (finalAttrs: {
          NIX_CFLAGS_LINK =
            toString (finalAttrs.NIX_CFLAGS_LINK or "")
  ...
  overrideSDK =
    ...
    let
      ...
      replacePropagatedFrameworks =
        ...
        let
          ...
          env = {
            ...
            pkgOutputs = lib.concatMapStrings (
              output: "${output}\t${(lib.getOutput output pkg).outPath}\n"
            ) pkg.outputs;
            passAsFile = [
              "dependencies"
              "pkgOutputs"
        ...
        if lib.length propagatedInputs > 0 && propagatedInputs != mappedInputs then
          pkgs.runCommand pkg.name env ''
      ...
      mapRuntimeToSDK =
        ...
        if pkg != null && lib.isAttrs pkg && lib.getName pkg == "xcodebuild" then
          pkg.override { stdenv = overrideSDK stdenv { inherit darwinMinVersion darwinSdkVersion; }; }
        else
          pkg;
