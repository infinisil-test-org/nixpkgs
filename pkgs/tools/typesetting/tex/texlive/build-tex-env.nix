...
lib.fix (
  ...
  let
    ...
    splitOutputs =
      ...
      // (lib.genAttrs pkgList.nonEnvOutputs (
        outName:
        (buildEnv {
          inherit name;
    ...
    passthru =
      lib.optionalAttrs (!__combine) (splitOutputs // { all = builtins.attrValues splitOutputs; })
      // {
        ...
        postBuild =
          ...
          +
            ...
            (
              let
                hyphens = lib.filter (
                  p: p.hasHyphens or false && p.tlOutputName or p.outputName == "tex"
                ) pkgList.nonbin;
                ...
                scriptLua = writeText "hyphens.lua.sed" (
                  "1{ s/^(-- Generated by .*)$/\\1, modified by ${
                    if __combine then "texlive.combine" else "Nixpkgs"
                  }/; p; }\n"
        ...
        (
          ...
          // lib.optionalAttrs (!__combine) (
            ...
            // builtins.listToAttrs (
              map (out: {
                name = "otherOutput_" + out;
                value = splitOutputs.${out};
              }) pkgList.nonEnvOutputs
            )
          )
        );
