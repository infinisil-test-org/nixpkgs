...
let
  ...
  mainDrv =
    if hasBinfiles then
      bin
    else if hasRunfiles then
      tex
    else if hasTlpkg then
      tlpkg
    else if hasDocfiles then
      texdoc
    else if hasSource then
      texsource
    else
      tex; # fall back to attrset tex if there is no derivation
  ...
  tex =
    if hasRunfiles then
      mkContainer "run" "tex" sha512.run
    else
      passthru
      // {
        inherit meta;
    ...
    runCommand "${name}"
      {
        ...
        scriptsFolder = lib.optionals (tex ? outPath) (
          builtins.map (f: tex.outPath + "/scripts/" + f) (lib.toList args.scriptsFolder or pname)
        );
        # binaries info
  ...
  man =
    builtins.removeAttrs (runCommand "${name}-man"
      {
        inherit meta texdoc;
        passthru = passthru // {
          tlOutputName = "man";
  ...
  info =
    builtins.removeAttrs (runCommand "${name}-info"
      {
        inherit meta texdoc;
        passthru = passthru // {
          tlOutputName = "info";
