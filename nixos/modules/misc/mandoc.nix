  ...
  config = lib.mkIf cfg.enable {
    environment = {
      ...
      extraSetup = lib.mkIf config.documentation.man.generateCaches ''
        ...
        } ${lib.concatMapStringsSep " " (path: lib.escapeShellArg path) cfg.settings.manpath}
        do
      ...
      profileRelativeSessionVariables."MANPATH" = map (
        path: if builtins.substring 0 1 path != "/" then "/${path}" else path
      ) cfg.manPath;
    };
  };
}
